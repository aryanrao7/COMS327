...
static int new_map()
{
  ...
  d = (abs(world.latest_index[dim_x] - (MAP_SIZE / 2)) +
       abs(world.latest_index[dim_y] - (MAP_SIZE / 2)));
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      ...
      if (world.pc.pos[dim_y] == y &&
        ...
        default:
          cell = ERROR_SYMBOL;
          color_code = 0; // Reset to default color
          default_reached = 1;
          break;
        }
...
static int32_t hikerComp(const void *key, const void *with) {
...
static int32_t rivalComp(const void *key, const void *with) {
...
int main(int argc, char *argv[])
{
  ...
  return 0;// remove if directions required
