                                     ...
                                     uint32_t count)
{
  ...
  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 40, "There are %d trainers:", count);
  mvprintw(4, 19, " %-40s ", *s);
  mvprintw(5, 19, " %-40s ", "");

  for (i = 0; i < count; i++) {
    snprintf(s[i], 40, "%16s %c: %2d %s and %2d %s",
  ...
  if (count <= 13) {
    ...
    mvprintw(count + 7, 19, " %-40s ", "Hit escape to go back");
    while (getch() != 27)
      ;//empty
  } else {
    mvprintw(19, 19, " %-40s ", "");
    mvprintw(20, 19, " %-40s ", "Scroll with arrows or hit escape to go back");
...
void io_pokemart()
{
  mvprintw(0, 0, "Welcome to the Pokemart! What do you want to buy?");
  refresh();
  getch();
}
...
void io_battle(character_t *aggressor, character_t *defender)
{
  ...
  io_display();
  mvprintw(0, 0, "You are going down!!!!");
  refresh();
  getch();
  if (aggressor->pc) {
    npc = defender;
...
void io_handle_input(pair_t dest)
{
  ...
  do {
    ...
    case 'q':
     mvprintw(0, 0, "Use Q to quit!");
     turn_not_consumed = 1;
     break;

    default:
      mvprintw(0, 0, "Key not found %#o ", key);
      turn_not_consumed = 1;
    }
    refresh();
