...
void io_teleport_world(pair_t dest)
{
  ...
  curs_set(1);
  do {
    mvprintw(0, 0, "Enter x coordinates:  ");
    refresh();
    mvscanw(0, 21, d_x, &x);
  } while (x < -200 || x > 200);
  do {
    mvprintw(0, 0, "Enter y coordinates:  ");
    refresh();
    mvscanw(0, 21, d_y, &y);
  } while (y < -200 || y > 200);
...
void io_encounter_pokemon()
{
  ...
  io_queue_message("%s%s%s: HP:%d ATCK:%d DEF:%d SPATCK:%d SPDEF:%d SPEED:%d %s",
...
void io_choose_starter()
{
  ...
  curs_set(1);
  do {
    mvprintw( 4, 20, "CHOOSE ONE FROM");
    mvprintw( 5, 20, "THE THREE POWERFUL POKEMONS");
    mvprintw( 6, 20, "TO TAKE ON YOUR JOURNEY");
    mvprintw(8, 20, "   1 %s", choice[0]->get_species());
    mvprintw(9, 20, "   2 %s", choice[1]->get_species());
    mvprintw(10, 20, "   3 %s", choice[2]->get_species());
    mvprintw(12, 20, "Choose 1, 2, or 3: ");
