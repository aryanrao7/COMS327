...
void db_parse(bool print, const char *file)
{
  FILE *f;
  char line[800];
  int i;
...
if (print && !strcmp(file,"pokemon")) {
  ...
  if (print && !strcmp(file,"moves")) {
    ...
    for (i = 1; i < 845; i++) {
        std::cout << i2s(moves[i].id) << "," << moves[i].identifier << "," << i2s(moves[i].generation_id) << ","
                  ...
                  << i2s(moves[i].accuracy) << "," << i2s(moves[i].priority) << "," << i2s(moves[i].target_id) << ","
                  << i2s(moves[i].damage_class_id) << "," << i2s(moves[i].effect_id) << ","
                  << i2s(moves[i].effect_chance) << "," << i2s(moves[i].contest_type_id) << ","
                  << i2s(moves[i].contest_effect_id) << "," << i2s(moves[i].super_contest_effect_id) << std::endl;
    }
  }
  ...
  if (print && !strcmp(file,"pokemon_moves")) {
  ...
  if (print && !strcmp(file,"pokemon_species")) {
    ...
    fclose(f);
       ...
       for (i = 1; i < 899; i++) {
        std::cout << i2s(species[i].id) << "," << species[i].identifier << ","
                  << i2s(species[i].generation_id) << "," << i2s(species[i].evolves_from_species_id) << ","
                  << i2s(species[i].evolution_chain_id) << "," << i2s(species[i].color_id) << ","
                  << i2s(species[i].shape_id) << "," << i2s(species[i].habitat_id) << ","
                  << i2s(species[i].gender_rate) << "," << i2s(species[i].capture_rate) << ","
                  << i2s(species[i].base_happiness) << "," << i2s(species[i].is_baby) << ","
                  << i2s(species[i].hatch_counter) << "," << i2s(species[i].has_gender_differences) << ","
                  << i2s(species[i].growth_rate_id) << "," << i2s(species[i].forms_switchable) << ","
                  << i2s(species[i].is_legendary) << "," << i2s(species[i].is_mythical) << ","
                  << i2s(species[i].order) << "," << i2s(species[i].conquest_order) << std::endl;
    }
  }
  ...
  if (print && !strcmp(file,"experience")) {
  ...
  if (print && !strcmp(file,"type_names")) {
  ...
  if (print && !strcmp(file,"pokemon_stats")) {
  ...
  if (print && !strcmp(file,"stats")) {
    ...
    for (i = 1; i < 9; i++) {
        std::cout << i2s(stats[i].id) << "," << i2s(stats[i].damage_class_id) << ","
                  << stats[i].identifier << "," << i2s(stats[i].is_battle_only) << ","
  ...
  if (print) {
  if (print && !strcmp(file,"pokemon_types")) {
        for (i = 1; i < 1676; i++) {
      fprintf(f, "%s,%s,%s\n",
