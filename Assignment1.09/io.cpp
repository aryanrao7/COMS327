...
void io_display()
{
  ...
  mvprintw(22, 30, "Latest visible trainer: ");
                                     ...
                                     uint32_t count)
{
  ...
  if (count <= 13) {
    ...
    mvprintw(count + 7, 19, " %-40s ", "Enter Esc to continue.");
    while (getch() != 27)
      ;
  } else {
    mvprintw(19, 19, " %-40s ", "");
    mvprintw(20, 19, " %-40s ",
             "Scroll through arrows and enter Esc to continue.");
...
void io_pokemart()
{
  mvprintw(0, 0, "Welcome to Pokemart!!!");
...
{
    mvprintw(0, 0, "Welcome to the Poke Center. Your pokemons are taken care of!");
    
    const int numPokemons = 6;
    for (int i = 0; i < numPokemons; ++i) {
        if (world.pc.pokemon[i]) { 
            world.pc.pokemon[i]->cur_hp = world.pc.pokemon[i]->get_hp();
        }
    }
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == ter_water) {
    return 1;
  }
  ...
  for(i = 0; i<6; i++){
    if(world.pc.pokemon[i]){
      if(is_alive(i)){
        ...
        mvprintw(i+3,0,"%d) %s: killed!",i+1, world.pc.pokemon[i]->get_species());
      }
    }else{
      break;
    }
...
int io_switch_pokemon_forced(){
  clear();
  io_list_pokemon();
  mvprintw(1,0, "Select the pokemon you want to switch?");
  refresh();
  bool deciding = true;
  char c;
  while(deciding){
      ...
      case '1':
        if(world.pc.pokemon[0]){
          if(world.pc.pokemon[0]->cur_hp < 1){
            mvprintw(19,0,"Cannot Switch. It's already dead!");
          }else{
            deciding = false;
            return 0;
          }
        } else{
          mvprintw(20,0,"Invalid Pokemon!");
        }
        break;
      case '2':
        if(world.pc.pokemon[1]){
          ...
          mvprintw(20,0,"Please slect a valid Pokemon!");
        }
        break;
      case '3':
...
int do_move(int p_move, Pokemon *pc_poke, Pokemon *enemy){
  ...
  mvprintw(21,0, "Enter any Key to continue");
  refresh();
  int i;
  ...
  if(pc_first){
    ...
    if(!pc_miss){
      ...
      if(enemy->cur_hp <= 0){
        enemy->cur_hp = 0;
        mvprintw(11,40, "%s Was killed!", enemy->get_species());
        refresh();
        getch();
        return 1;
      }
    ...
    if(!enemy_miss){
      int damage_incoming = get_move_damage(enemy->get_level() * 1.0, moves[enemy_move].power * 1.0, enemy->get_atk() * 1.0, pc_poke->get_def() * 1.0, enemy->get_speed() * 1.0, 1.0); 
      ...
      if(pc_poke->cur_hp <= 0){
        pc_poke->cur_hp = 0;
        mvprintw(11,0, "%s Was killed!", pc_poke->get_species());
        refresh();
        getch();
        return 2;
      }
    ...
    if(!pc_miss){
      int damage = get_move_damage(pc_poke->get_level() * 1.0, moves[p_move].power * 1.0, pc_poke->get_atk() * 1.0, enemy->get_def() * 1.0, pc_poke->get_speed() * 1.0, 1.0); 
      mvprintw(10,0, "%s Hit for %d!", moves[p_move].identifier, damage);
      refresh();
      getch();
...
int io_post_knockout_logic(){
  ...
  if(i == 6){
    mvprintw(0,0,"Your Last Pokemon is killed. You can revive them from your bag!");
    mvprintw(21,0, "Press any Key to continue");
    refresh();
    getch();
    return -1; 
  } else{
    mvprintw(0,0,"Your Pokemon has been killed, Please choose another one! Thanks");
    mvprintw(21,0, "Press any Key to continue");
    refresh();
    getch();
      ...
      case '4':
        if(move_count > 3){
          ...
          if(move_result == 1 && wild){
            attempt_capture(enemy);
            clear();
            mvprintw(0,0,"%s Will be added to your bag if enough space!", enemy->get_species());
            mvprintw(21,0, "Press any Key to continue");
            refresh();
            getch();
            return 100;
...
int io_enter_bag(bool in_wild_battle){
  ...
  char c;
  while(in_bag){
    mvprintw(0,0,"Welcome to the bag!");
    mvprintw(1,0,"Please select what do you want to choose.");

    mvprintw(3,0,"1) Give life: %d", world.pc.bag[revive]);
    mvprintw(4,0,"2) Health: %d", world.pc.bag[potion]);
    mvprintw(5,0,"3) Pokeball: %d (Capture if in battle)", world.pc.bag[pokeball]);
    mvprintw(21,0, "Esc and leave the bag");
    refresh();
    ...
    if(c == '1' && world.pc.bag[revive] > 0){
      clear();
      io_list_pokemon();
      mvprintw(1,0,"Select Pokemon to give life");
      mvprintw(21,0, "ESC to go back");
      refresh();
      bool deciding = true;
      int rev_target = 0;
          ...
          case '1':
            if(world.pc.pokemon[0]){
              if(world.pc.pokemon[0]->cur_hp > 0){
                mvprintw(19,0,"Pokemon is not killed yet");
              }else{
                deciding = false;
                rev_target = 0;
              }
          ...
          case '1':
            if(world.pc.pokemon[0]){
              if(world.pc.pokemon[0]->cur_hp == world.pc.pokemon[0]->get_hp()){
                mvprintw(19,0,"You cannot use the health potion on healthy pokemon");
              }else{
                deciding = false;
                pot_target = 0;
              }
      ...
      if(in_wild_battle){
        if(world.pc.bag[pokeball] > 0){
          ...
          clear();
          mvprintw(20,0, "No more pokeballs");
        }
      } else{
        clear();
        mvprintw(20,0, "You can use pokeballs with only wild pokemon");
      }
...
void io_encounter_pokemon()
{
  ...
  clear();
  while(fighting){
    mvprintw(0, 0, "A wild %s has appeared!", p->get_species());
    mvprintw(15,0, "Current Pokemon: %s", world.pc.pokemon[cur_pokemon]->get_species());
    mvprintw(16,0, "It is Level: %d    HP: %d/%d", world.pc.pokemon[cur_pokemon]->get_level(), world.pc.pokemon[cur_pokemon]->cur_hp, world.pc.pokemon[cur_pokemon]->get_hp());
    mvprintw(15,40, "Opponent Pokemon: %s", p->get_species());
    mvprintw(16,40, "It is Level: %d    HP: %d/%d", p->get_level(), p->cur_hp, p->get_hp());
    mvprintw(3, 0, "1) Let's fight");
    mvprintw(4, 0, "2) Bag");
    mvprintw(5, 0, "3) Let's Run");
    mvprintw(6, 0, "4) This pokemon is too weak and not supportive. Switch Pokemon");
    refresh(); 
    ...
    if(c == '1'){ 
      ...
      if(attempt_run(world.pc.pokemon[cur_pokemon]->get_speed(), p->get_speed(), attempts) == true){
        ...
        attempts++;
        clear();
        mvprintw(19, 0, "You cannot escape!");
        mvprintw(21,0, "Press any Key to continue...");
        refresh();
        getch();
  ...
  for(i = 0; i<6; i++){
    if(enemy->pokemon[i]){
      if(is_enemy_poke_alive(enemy->pokemon[i])){
        ...
        mvprintw(i+3,50,"%s: killed!", enemy->pokemon[i]->get_species());
      }
    }else{
      break;
    }
...
void io_battle(Character *enemy)
{
  ...
  clear();
  while(fighting){
    mvprintw(0, 0, "Trainer challenged you for fight!");
    mvprintw(15,0, "Current Pokemon: %s", world.pc.pokemon[cur_pokemon]->get_species());
